          ...
          <table className="w-full">
            <thead>
              <tr className="bg-zinc-50 border-b border-zinc-100">
                {["สมาชิก", "งาน", "บัตร", "วันที่", "สถานะ", ""].map((h) => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-10 text-sm text-zinc-400">ไม่มีรายการ</td></tr>
              ) : filtered.map((t) => (
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-md mx-4 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100">
              <div>
                ...
                <p className="text-[10px] text-zinc-400 mt-0.5">{selected.profiles?.display_name ?? selected.profiles?.username} · {selected.events?.title}</p>
              </div>
