            ...
            return (
              <div key={ev.id} className="card overflow-hidden">
                <div className="px-4 py-4">
                  ...
                  <button onClick={() => !full && setSelectedEvent(ev)} disabled={full}
                    className={`mt-3 w-full text-xs font-semibold py-2.5 rounded-xl tracking-wide ${full ? "bg-zinc-100 text-zinc-400 cursor-not-allowed" : "bg-zinc-900 text-white active:opacity-70"}`}>
      ...
      {activeTab === "จองโต๊ะ" && (
        <div className="px-4 py-4">
          <div className="card p-5">
            ...
            <p className="text-[11px] text-zinc-400 mb-4">เปิดให้จองทุกวัน 11:00 – 21:00 น.</p>
            <div className="space-y-4">
              ...
              <div>
                ...
                <select className="input text-sm"><option>1 คน</option><option>2 คน</option><option>3 คน</option><option>4 คน</option></select>
              </div>
              {tableDone ? (
                <div className="bg-zinc-50 rounded-2xl p-4 text-center">
                  <div className="text-2xl mb-2">✓</div>
                  <p className="text-sm font-semibold text-zinc-900">จองสำเร็จ!</p>
                  <p className="text-[11px] text-zinc-400 mt-1">{tableDate} เวลา {selectedSlot} น.</p>
                </div>
              ) : (
                <button onClick={() => tableDate && selectedSlot && setTableDone(true)} disabled={!tableDate || !selectedSlot}
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-3xl px-5 py-6">
            ...
            <div className="flex gap-2 mt-4">
              ...
              <button onClick={() => setBookingDone(true)} className="btn-primary flex-1 py-3">ยืนยัน</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-3xl px-5 py-8 text-center">
            <div className="w-14 h-14 bg-zinc-900 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M5 12l5 5L19 7" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </div>
            <h3 className="text-base font-bold text-zinc-900 mb-5">ลงทะเบียนสำเร็จ!</h3>
            <button onClick={() => { setBookingDone(false); setSelectedEvent(null); }} className="btn-primary w-full py-3">กลับ</button>
          </div>
        </div>
      )}
